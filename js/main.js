...
function initCarouselAutoLoop(trackId, intervalMs = 3000) {
  ...
  Array.from({ length: 5 }, () => originals).flat().forEach((original, index) => {
    const item = index >= count * 2 && index < count * 3 ? original : original.cloneNode(true);
  ...
  function startTimer() {
    carouselTimers[trackId] = setInterval(() => moveCarousel(state, 1), intervalMs);
  }
...
function renderHero() {
  ...
  if (!container || !banners.heroSlides) return;

  const slides = banners.heroSlides.sort((a, b) => a.order - b.order);
  ...
  return `
    <div class="product-card fade-in" data-id="${product.id}">
      <div class="product-image-wrap">
        ...
        <img src="${product.image}" alt="${product.name}" loading="lazy" onclick="openProductModal(${product.id})" style="cursor:pointer">
        <div class="product-actions-overlay">
          <button class="quick-action-btn" onclick="openProductModal(${product.id})" title="Quick View">
            <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z"/><circle cx="12" cy="12" r="3"/></svg>
          </button>
          <button class="quick-action-btn" onclick="toggleWishlist(${product.id})" title="Wishlist">
      ...
      <div class="product-info">
        ...
        <div class="product-price">
          ...
          ${hasSale ? `<span class="price-original">$${product.price.toFixed(2)}</span>` : ''}
        </div>
...
function renderBestSellers() {
  ...
  if (!container) return;
  const best = products.filter(p => p.bestSeller && p.status === 'Active');
  container.innerHTML = best.map(p => createProductCard(p)).join('');
}
...
function renderJewelry() {
  const container = document.getElementById('jewelryCollection');
  if (!container) return;
  const jewelry = products.filter(p => p.category === 'Jewelry' && p.status === 'Active');
  container.innerHTML = jewelry.map(p => createProductCard(p)).join('');
}

function renderNails() {
  const container = document.getElementById('nailStudio');
  if (!container) return;
  const nails = products.filter(p => p.category === 'Nails' && p.status === 'Active');
  container.innerHTML = nails.map(p => createProductCard(p)).join('');
}
...
function renderTestimonials() {
  ...
  if (!reviews || reviews.length === 0) {
    container.innerHTML = '<p style="padding:20px;color:var(--gray-400)">No reviews available.</p>';
    return;
  }
  container.innerHTML = reviews.map(r => {
    const starCount = typeof r.rating === 'number' ? r.rating : 5;
    const stars = '★'.repeat(starCount) + '☆'.repeat(5 - starCount);
    const initials = r.initials || (r.author ? r.author.split(' ').map(n => n[0]).join('').toUpperCase() : 'U');
    return `
      <div class="testimonial-card">
        <div class="testimonial-stars">${stars}</div>
        <p class="testimonial-text">"${r.text}"</p>
        ${r.productName ? `<div style="font-size:0.8rem;color:var(--blush-600);font-weight:600;margin-bottom:12px">Product: ${r.productName}</div>` : ''}
        <div class="testimonial-author">
          <div class="testimonial-avatar">${initials}</div>
          <div class="testimonial-author-info">
            <h5>${r.author}</h5>
            <span>${r.subtitle || 'Verified Buyer'}</span>
          </div>
        </div>
      </div>
    `;
...
function initializeTestimonialsCarousel() {
  ...
  testimonialsCarouselState?.destroy?.();

  const originals = Array.from(track.children)
    .filter(slide => !slide.hasAttribute('data-testimonial-clone'));
  if (originals.length < 2) return;

  const reducedMotion = window.matchMedia('(prefers-reduced-motion: reduce)');
  const state = testimonialsCarouselState = {
    track,
    viewport,
  ...
  const appendSet = isOriginalSet => {
    originals.forEach(original => {
      const slide = isOriginalSet ? original : original.cloneNode(true);
      if (!isOriginalSet) {
        slide.setAttribute('data-testimonial-clone', 'true');
  ...
  appendSet(true);
  appendSet(false);
  track.classList.add('testimonials-transform-track');
  track.style.scrollSnapType = 'none';
  ...
  function normalize() {
    if (state.index < state.count) state.index += state.count;
    if (state.index >= state.count * 2) state.index -= state.count;
    state.logicalIndex = (state.index - state.count + state.count) % state.count;
    position(false);
  }
  ...
  function finish() {
    ...
    if (state.index < state.count || state.index >= state.count * 2) normalize();
    if (state.queuedDirection) {
      const direction = state.queuedDirection > 0 ? 1 : -1;
  ...
  function move(direction) {
    if (state.isAnimating) {
      state.queuedDirection += direction > 0 ? 1 : -1;
      return;
    }
    state.index += direction > 0 ? 1 : -1;
    state.logicalIndex = (state.index - state.count + state.count) % state.count;
    ...
    position(true);
    clearTimeout(state.fallbackTimer);
    state.fallbackTimer = setTimeout(finish, reducedMotion.matches ? 0 : 700);
  }
  ...
  function resumeAutoplay() {
    if (reducedMotion.matches || state.autoplayTimer) return;
    state.autoplayTimer = setInterval(() => move(1), 4000);
  }
...
function renderPromoBanner() {
  ...
  if (!container) return;
  const pb = banners.promoBanner;
  if (!pb || !pb.enabled) { container.classList.add('hidden'); return; }

  const now = new Date();
  const start = new Date(pb.startDate);
  const end = new Date(pb.endDate);
  if (now < start || now > end) { container.classList.add('hidden'); return; }

  container.innerHTML = `
    <div class="promo-banner">
      <div class="promo-banner-bg">
        <img src="${pb.backgroundImage}" alt="${pb.title}">
      </div>
      <div class="promo-banner-content">
        <h3>${pb.title}</h3>
        <p>${pb.subtitle}</p>
        <a href="${pb.buttonLink}" class="btn-primary">${pb.buttonText}</a>
      </div>
    </div>
  `;
}
...
function renderBlog() {
  ...
  const displayedPosts = isBlogPage ? published : published.slice(0, 3);
  ...
  container.innerHTML = displayedPosts.map(post => `
    <article class="blog-card">
      ...
      <div class="blog-card-content">
        ...
        <p class="blog-card-excerpt">${post.content.substring(0, 120).replace(/[#*\n]/g, '')}...</p>
        <span class="read-more" onclick="openBlogModal(${post.id})">Read More →</span>
      </div>
    </article>
  `).join('');
}
...
function renderFilterSidebar() {
  ...
  container.innerHTML = categories.map(cat => `
    <label class="filter-option">
      <input type="checkbox" value="${cat.name}" onchange="toggleCategoryFilter(this)">
      ${cat.name}
      <span style="margin-left:auto;color:var(--gray-400);font-size:0.8rem">${products.filter(p => p.category === cat.name).length}</span>
    </label>
  `).join('');
  ...
  if (priceSlider) {
    ...
    priceSlider.value = shopFilters.maxPrice;
    priceSlider.oninput = function() {
      shopFilters.maxPrice = parseInt(this.value);
      document.getElementById('priceDisplay').textContent = `Up to $${this.value}`;
      renderShop();
    };
  }
}
...
function toggleCategoryFilter(checkbox) {
  if (checkbox.checked) {
    shopFilters.categories.push(checkbox.value);
  } else {
    shopFilters.categories = shopFilters.categories.filter(c => c !== checkbox.value);
  }
  renderShop();
}
...
function filterByCategory(category) {
  ...
  renderShop();
  // Check the checkbox
  document.querySelectorAll('#filterCategories input').forEach(cb => {
...
function saveCart() {
  localStorage.setItem('luxbeauty_cart', JSON.stringify(cart));
}
...
function toggleWishlist(productId) {
  ...
  localStorage.setItem('luxbeauty_wishlist', JSON.stringify(wishlist));
  updateWishlistBadge();
    ...
    return `
      <div class="cart-item">
        ...
        <div class="cart-item-info">
          ...
          <div class="cart-item-price">$${price.toFixed(2)}</div>
          <div class="cart-item-controls">
            <button class="add-to-cart-btn" style="width:auto;padding:8px 20px;font-size:0.75rem" onclick="addToCart(${p.id})">Add to Cart</button>
  ...
  body.innerHTML = `
    ...
    <div class="modal-info-section">
      ...
      <div class="modal-price-row">
        <span class="price-current">$${displayPrice.toFixed(2)}</span>
        ${hasSale ? `<span class="price-original">$${p.price.toFixed(2)}</span>` : ''}
      </div>
      <p class="modal-description">${p.description}</p>
      <div class="modal-meta">
        ${p.brand ? `<div class="modal-meta-item"><span class="modal-meta-label">Brand</span><span class="modal-meta-value">${p.brand}</span></div>` : ''}
        ${p.sku ? `<div class="modal-meta-item"><span class="modal-meta-label">SKU</span><span class="modal-meta-value">${p.sku}</span></div>` : ''}
        ${p.material ? `<div class="modal-meta-item"><span class="modal-meta-label">Material</span><span class="modal-meta-value">${p.material}</span></div>` : ''}
        ${p.color ? `<div class="modal-meta-item"><span class="modal-meta-label">Color</span><span class="modal-meta-value">${p.color}</span></div>` : ''}
        ${p.collection ? `<div class="modal-meta-item"><span class="modal-meta-label">Collection</span><span class="modal-meta-value">${p.collection}</span></div>` : ''}
        ${p.size ? `<div class="modal-meta-item"><span class="modal-meta-label">Size</span><span class="modal-meta-value">${p.size}</span></div>` : ''}
        ${p.finish ? `<div class="modal-meta-item"><span class="modal-meta-label">Finish</span><span class="modal-meta-value">${p.finish}</span></div>` : ''}
        ${p.shape ? `<div class="modal-meta-item"><span class="modal-meta-label">Shape</span><span class="modal-meta-value">${p.shape}</span></div>` : ''}
        ${p.length ? `<div class="modal-meta-item"><span class="modal-meta-label">Length</span><span class="modal-meta-value">${p.length}</span></div>` : ''}
        ${p.skinType ? `<div class="modal-meta-item"><span class="modal-meta-label">Skin Type</span><span class="modal-meta-value">${p.skinType}</span></div>` : ''}
        ${p.ingredients ? `<div class="modal-meta-item"><span class="modal-meta-label">Ingredients</span><span class="modal-meta-value">${p.ingredients}</span></div>` : ''}
        ${p.benefits ? `<div class="modal-meta-item"><span class="modal-meta-label">Benefits</span><span class="modal-meta-value">${p.benefits}</span></div>` : ''}
        ${p.usage ? `<div class="modal-meta-item"><span class="modal-meta-label">How to Use</span><span class="modal-meta-value">${p.usage}</span></div>` : ''}
        ${p.shades && p.shades.length ? `<div class="modal-meta-item"><span class="modal-meta-label">Shades</span><span class="modal-meta-value">${p.shades.join(', ')}</span></div>` : ''}
        <div class="modal-meta-item"><span class="modal-meta-label">Stock</span><span class="modal-meta-value">${p.stock > 0 ? p.stock + ' available' : 'Out of stock'}</span></div>
      </div>
      <div class="modal-actions">
        ...
        <button class="btn-primary" style="flex:1;animation:none" onclick="addToCart(${p.id}, parseInt(document.getElementById('modalQty').textContent)); closeProductModal();">Add to Cart</button>
      </div>
    </div>
  `;
...
function openBlogModal(postId) {
  ...
  const content = post.content
    .replace(/## (.*)/g, '<h3 style="font-family:var(--font-display);margin:20px 0 10px;color:var(--charcoal)">$1</h3>')
    .replace(/\*\*(.*?)\*\*/g, '<strong>$1</strong>')
    .replace(/\n/g, '<br>');

  body.innerHTML = `
    <div style="aspect-ratio:16/9;overflow:hidden;border-radius:var(--radius-lg) var(--radius-lg) 0 0">
    ...
    <div style="padding:40px">
      <div class="blog-meta" style="margin-bottom:16px">
        <span style="color:var(--gold-500);font-weight:600">${post.category}</span>
        <span>${formatDate(post.publishDate)}</span>
        <span>By ${post.author}</span>
      </div>
      <h2 style="font-family:var(--font-display);font-size:1.8rem;margin-bottom:20px;color:var(--charcoal)">${post.title}</h2>
      <div style="font-size:0.95rem;color:var(--gray-500);line-height:1.8">${content}</div>
    </div>
  `;
...
function renderFooter() {
  ...
  if (socialContainer) {
    const icons = {
      reddit: '<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.7" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="8"/><path d="M8.5 13.5c.8 1.1 2 1.7 3.5 1.7s2.7-.6 3.5-1.7M9 10.5h.01M15 10.5h.01M14.5 5.5l1.2-2.5 2.8.8"/></svg>',
      pinterest: '<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="12" cy="12" r="10"/><path d="M8.5 20.5c1-3 2-8 2-8"/><path d="M12 12a3 3 0 1 0-3-3"/><path d="M12 12a3 3 0 0 1 3 3c0 2-1 3-2 3"/></svg>'
    };
        socialContainer.innerHTML = ['reddit', 'pinterest'].map(platform => {
          const url = sm[platform];
          return url ? `<a href="${url}" target="_blank" title="${platform}">${icons[platform]}</a>` : '';
        }).join('');
  }
...
function showToast(message, type = 'success') {
  ...
  document.querySelectorAll('.lux-notify, .lux-notify-backdrop').forEach(el => el.remove());
  document.documentElement.classList.add('lux-notify-open');

  const msg = message.toLowerCase();

  // Context-aware icons and copy
  let iconSvg, iconClass, title, subtitle;

  if (msg.includes('added to cart') || (msg.includes('cart') && !msg.includes('empty'))) {
    iconClass = 'icon-blush';
    iconSvg = `<svg fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round" viewBox="0 0 24 24"><path d="M6 2L3 6v14a2 2 0 002 2h14a2 2 0 002-2V6l-3-4z"/><line x1="3" y1="6" x2="21" y2="6"/><path d="M16 10a4 4 0 01-8 0"/></svg>`;
    title = 'Added to Cart';
    subtitle = 'Your item is ready for checkout.';
  } else if (msg.includes('added to wishlist') || (msg.includes('wishlist') && !msg.includes('removed'))) {
    iconClass = 'icon-blush';
    iconSvg = `<svg fill="currentColor" stroke="none" viewBox="0 0 24 24"><path d="M20.84 4.61a5.5 5.5 0 00-7.78 0L12 5.67l-1.06-1.06a5.5 5.5 0 00-7.78 7.78l1.06 1.06L12 21.23l7.78-7.78 1.06-1.06a5.5 5.5 0 000-7.78z"/></svg>`;
    title = 'Saved to Wishlist';
    subtitle = 'Added to your personal collection.';
  } else if (msg.includes('removed from wishlist')) {
    iconClass = 'icon-blush';
    iconSvg = `<svg fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round" viewBox="0 0 24 24"><path d="M20.84 4.61a5.5 5.5 0 00-7.78 0L12 5.67l-1.06-1.06a5.5 5.5 0 00-7.78 7.78l1.06 1.06L12 21.23l7.78-7.78 1.06-1.06a5.5 5.5 0 000-7.78z"/></svg>`;
    title = 'Removed from Wishlist';
    subtitle = 'Item removed from your collection.';
  } else if (msg.includes('subscrib') || msg.includes('thank you for')) {
    iconClass = 'icon-gold';
    iconSvg = `<svg fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round" viewBox="0 0 24 24"><path d="M4 4h16c1.1 0 2 .9 2 2v12c0 1.1-.9 2-2 2H4c-1.1 0-2-.9-2-2V6c0-1.1.9-2 2-2z"/><polyline points="22,6 12,13 2,6"/></svg>`;
    title = 'Welcome to Veloura';
    subtitle = message;
  } else if (msg.includes('empty') || type === 'error') {
    iconClass = 'icon-error';
    iconSvg = `<svg fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round" viewBox="0 0 24 24"><path d="M6 2L3 6v14a2 2 0 002 2h14a2 2 0 002-2V6l-3-4z"/><line x1="3" y1="6" x2="21" y2="6"/><line x1="12" y1="11" x2="12" y2="15"/><line x1="12" y1="19" x2="12.01" y2="19"/></svg>`;
    title = 'Cart is Empty';
    subtitle = 'Add some products before checking out.';
  } else if (msg.includes('checkout') || msg.includes('demo')) {
    iconClass = 'icon-gold';
    iconSvg = `<svg fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round" viewBox="0 0 24 24"><polyline points="20 6 9 17 4 12"/></svg>`;
    title = 'Thank You!';
    subtitle = message;
  } else {
    iconClass = 'icon-gold';
  ...
  backdrop.className = 'lux-notify-backdrop';

  // Build card
  const card = document.createElement('div');
  card.className = 'lux-notify';
  card.innerHTML = `
    <div class="lux-notify-icon ${iconClass}">${iconSvg}</div>
    <div class="lux-notify-rule"></div>
    <p class="lux-notify-title">${title}</p>
    ${subtitle ? `<p class="lux-notify-sub">${subtitle}</p>` : ''}
  `;
  ...
  function dismiss() {
    clearTimeout(timer);
    backdrop.classList.add('lux-notify-exit');
    card.classList.add('lux-notify-exit');
    setTimeout(() => {
      backdrop.remove();
      card.remove();
      document.documentElement.classList.remove('lux-notify-open');
    }, 380);
...
function setupEventListeners() {
  ...
  if (cartOverlay) cartOverlay.addEventListener('click', closeDrawers);
  ...
  if (wishlistClose) wishlistClose.addEventListener('click', closeDrawers);
  ...
  if (blogModalOverlay) blogModalOverlay.addEventListener('click', (e) => {
  ...
  if (mobileBtn) mobileBtn.addEventListener('click', () => mobileMenu.classList.add('open'));
  ...
  if (checkoutBtn) {
    checkoutBtn.addEventListener('click', () => {
      ...
      showToast('Checkout is a demo in this static site. Thank you!', 'success');
    });
  }
}
...
function setupScrollAnimations() {
  ...
  document.querySelectorAll('.section-header, .category-card, .product-card, .blog-card').forEach(el => {
    el.style.opacity = '0';
...
// ===== Theme System =====
function initTheme() {
  // Read saved preference (flash-prevention script in <head> already applied it)
  const saved = localStorage.getItem('luxbeauty-theme') || 'light';
...
function applyTheme(theme) {
  ...
  localStorage.setItem('luxbeauty-theme', theme);
...
function updateThemeToggle(theme) {
  ...
  if (theme === 'dark') {
    ...
    if (sunIcon)  { sunIcon.style.opacity = '1';  sunIcon.style.transform = 'rotate(0deg) scale(1)'; }
    if (moonIcon) { moonIcon.style.opacity = '0'; moonIcon.style.transform = 'rotate(90deg) scale(0.5)'; }
  } else {
    btn.setAttribute('aria-label', 'Switch to dark theme');
    if (sunIcon)  { sunIcon.style.opacity = '0';  sunIcon.style.transform = 'rotate(90deg) scale(0.5)'; }
    if (moonIcon) { moonIcon.style.opacity = '1'; moonIcon.style.transform = 'rotate(0deg) scale(1)'; }
  }
}
...
function initCustomSortDropdown() {
  ...
  selects.forEach(select => {
    ...
    Array.from(select.options).forEach(opt => {
      ...
      li.className = 'custom-sort-option' + (opt.selected ? ' selected' : '');
    ...
    overlay.addEventListener('click', (e) => {
      ...
      document.querySelectorAll('.custom-sort-menu').forEach(m => m.classList.remove('open'));
